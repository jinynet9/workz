...
int net_default_route_available ( void );
...
#define UCI_GET_INT(N, V)						\
    do {								\
	char _v[40];							\
	shell_read_tnl ( _v, sizeof(_v), UCI" get %s", N ); 		\
	V = atoi ( _v );						\
    } while (0)
...
#define UCI_SET_INT(N, V)						\
    do {								\
	SYSTEM ( UCI" set %s=\'%d\'", N, V );				\
    } while (0)

#define UCI_COMMIT(arg...)						\
    do {								\
	SYSTEM ( UCI" commit " arg );					\
    } while (0)
