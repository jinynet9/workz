 ...
 */
int
net_default_route_available ( void )
{
    int rv;

    rv = SYSTEM ( "ip route show | grep default > /dev/null" );
    if ( rv == 0 )
	return true;

    rv = SYSTEM ( "ip -f inet6 route show | grep default > /dev/null" );
    if ( rv == 0 )
	return true;
